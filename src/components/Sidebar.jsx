...
import { Home, Rocket, Globe, Settings, LogOut, Star, User, Menu, X } from "lucide-react";
import logo from "../pages/OrbitSpace-Logo.svg"; 

function Sidebar() {
  const location = useLocation();
  const [isProfileMenuOpen, setIsProfileMenuOpen] = useState(false);
  const [isMobileOpen, setIsMobileOpen] = useState(false); // Nuevo estado para móvil
  ...
  // Cerrar menú móvil cuando cambias de ruta
  useEffect(() => {
  ...
  return (
    <>
      ...
      <button 
        ...
        className="fixed top-4 left-4 z-[60] p-2 bg-[#1e103c] border border-purple-500/30 rounded-lg text-white md:hidden"
      >
        {isMobileOpen ? <X size={24} /> : <Menu size={24} />}
      </button>

      {/* Overlay para móvil (oscurece el fondo al abrir) */}
      {isMobileOpen && (
        <div 
      ...
      {/* Sidebar Principal */}
      <aside className={`
        fixed left-0 top-0 h-screen w-64 bg-[#1e103c]/90 md:bg-[#1e103c]/40 backdrop-blur-xl border-r border-purple-500/20 
        flex flex-col justify-between p-6 z-[50] transition-transform duration-300
      ...
      `}>
              ...
              {isProfileMenuOpen && (
                <div className="absolute right-0 top-full mt-3 w-60 bg-[#1a0b36] border border-purple-500/40 rounded-2xl shadow-[0_10px_30px_rgba(168,85,247,0.3)] z-[100] overflow-hidden">
                  <div className="p-4 border-b border-purple-500/20 bg-[#15092a]/50 flex items-center gap-3">
                    <div className="w-10 h-10 shrink-0 rounded-full bg-gradient-to-br from-purple-600 to-fuchsia-600 flex items-center justify-center font-bold text-white text-sm">
                      GV
                    </div>
                    <div>
        ...
        <div className="text-[10px] text-purple-400/40 text-center pb-2">
           OrbitSpace© 2026
        </div>
      </aside>
    </>
